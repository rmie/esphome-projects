...
### Added
- **Type-Safe State Variables (Proposal State)**: Added support for arbitrary, compile-time typed state variables associated with each proposal (`state_vars` schema). Supported types include `float`, `double`, `bool`, and fixed-width integers (`int8_t` through `uint32_t`).
